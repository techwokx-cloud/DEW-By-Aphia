...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.action === "approve") {
    const existing = (await listContentPosts()).find((p) => p.id === id);
    if (!existing) return NextResponse.json({ error: "Not found" }, { status: 404 });

    const imageUrl = new URL(existing.image, getPublicSiteUrl()).toString();
    ...
    const post = await updateContentPost(id, { status: anySent ? "posted" : "approved" });
    return NextResponse.json({ item: post, instagram, facebook, threads });
  }

  if (body.action === "reject") {
    const post = await updateContentPost(id, { status: "rejected" });
